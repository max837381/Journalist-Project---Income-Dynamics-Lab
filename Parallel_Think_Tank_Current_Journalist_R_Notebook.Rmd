...
filename <- "think_tank_combined.csv"
...
tic()
df$nwords <- parSapply(cl, df$speech, count_number_words)
toc()
...
#df[!duplicated(df$id), ] 
df <- df[sample(nrow(df),size=1500),] #congress_observations # 15000
df$id <- factor(df$id)
```
...
bigrams <- function(df) {
  ...
  df <- corpus(df)
  df <- tokenizers::tokenize_words(df, lowercase = TRUE,) %>%
  ...
  # tokenz[3]
  df <- tokens_remove(df, pattern = stopwords("en"))
  # tokenz[3]
  df <- tokens_wordstem(df, language = "en")
  # tokenz[3]
  df <- tokens_ngrams(df, n = 2, concatenator = " ")
...
test <- sapply(list(df$text,df$doc_id), bigrams)
tic()
df1 <- bigrams(df2)
...
clusterEvalQ(cl, c(library(quanteda), library(dplyr), library(stringr), library(tokenizers), library(tictoc)))

tic()
df <- parSapply(cl, replicate(no_cores, df2, simplify=FALSE), bigrams)
toc()
...
#df <- sapply(list(df2), bigrams)

#df <- parSapply(cl, df2, bigrams(df2,text,doc_id))
...
# #DF <- DF[c(1:4)]
tic("data.table group by")
DT <- data.table(x)
data_sum <- DT[ , .(Frequency = sum(count)), by = list(speech_id,term)]     # Aggregate data
data_sum

toc()
# 
# tic("tidyverse group by")
# x <- x %>%
...
x <- data.frame(x[order(x$speech_id, x$term),])

data_sum <- data.frame(data_sum[order(data_sum$speech_id, data_sum$term),])
...
write.csv(coefficients_lasso, "aicc__think_tank_coefficients_lasso.csv")
...
```{r}
match_strings <- function(x, string) {
  positions <- str_which(x, string)
  cbind(as.numeric(positions),x[positions])
}
```
