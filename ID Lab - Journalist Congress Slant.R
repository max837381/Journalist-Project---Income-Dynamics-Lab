...
library(pacman)
p_load(gamlr,tm, textir, wordcloud2, tidyverse, usethis)
...
# byspeaker data 114 and 113 (most recent)

# This contains speaker id | phrase (bigram) | count
txt_byspeaker1 <- "hein-daily/byspeaker_2gram_114.txt"
byspeaker1 <- read.table(txt_byspeaker1,sep="|",header=T)

txt_byspeaker2 <- "hein-daily/byspeaker_2gram_113.txt"
byspeaker2 <- read.table(txt_byspeaker2,sep="|",header=T)


# merge byspeaker files
byspeaker <- rbind(byspeaker1,byspeaker2)
...
byspeaker <- head(byspeaker, 500000)
...
# Change all NA to . as seen in lecture 6
#test[is.na(test)] <- "."



# Load speakermap files
speakermap_1_file <- "/Users/max/Documents/UCSC/Summer 2022/Journalist Project - Income Dynamics Lab/hein-daily/114_SpeakerMap.txt"
speakermap_1 <- read.table(speakermap_1_file,sep="|",header=T)

speakermap_2_file <- "/Users/max/Documents/UCSC/Summer 2022/Journalist Project - Income Dynamics Lab/hein-daily/113_SpeakerMap.txt"
speakermap_2 <- read.table(speakermap_2_file,sep="|",header=T)

# Merge speakermap files
speakermap <- rbind(speakermap_1,speakermap_2)
...
# Merge also seems to take a really long time
...
# Drop columns or bigrams with a lot of NAs (greater than 60% of observations)
test <- test[lapply(test, function(x) mean(is.na(x))) < 0.6]
...
test <- test %>% relocate(party, .before = `000 per`)
test <- test %>% relocate(name, .before = speakerid)
